    ...
    func halfCloseEcho(strategy: Sockets.TCP.Listener.Tests.Strategy) async throws {
        ...
        try await withThrowingDiscardingTaskGroup { group in
            ...
            group.addTask {
                ...
                // Read the payload.
                let n = try await conn.read(into: unsafe Memory.Buffer.Mutable(buf))
                ...
                let eof = try await conn.read(into: unsafe Memory.Buffer.Mutable(buf))
                ...
                _ = try await conn.write(from: unsafe Memory.Buffer(echo))
            ...
            group.addTask {
                ...
                _ = try await clientIO.write(to: descriptor, from: unsafe Memory.Buffer(UnsafeRawBufferPointer(wbuf)))
                ...
                let n = try await clientIO.read(from: descriptor, into: unsafe Memory.Buffer.Mutable(rbuf))
                ...
                let eof = try await clientIO.read(from: descriptor, into: unsafe Memory.Buffer.Mutable(rbuf))
