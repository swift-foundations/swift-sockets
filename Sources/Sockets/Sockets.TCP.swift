...
extension Sockets {
    ...
    /// represents a connected pair. Both compose with swift-io's blocking strategy
    /// via the shared-executor pattern — socket syscalls run on the `IO`'s executor
    /// thread, and byte-level I/O flows through the `IO` witness.
    ///
